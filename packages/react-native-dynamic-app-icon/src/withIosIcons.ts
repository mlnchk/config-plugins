// https://github.com/expo/expo/blob/be24eba574fc3749bb7be4d579762d911480b6ad/packages/%40expo/prebuild-config/src/plugins/icons/withIosIcons.ts
...
let counter = 0;
const getImageSetPath = () => `Images.xcassets/AppIcon-${counter}.appiconset`;
...
export async function setIconsAsync(icon: string, projectRoot: string) {
  counter++;
  ...
  await fs.ensureDir(join(iosNamedProjectRoot, getImageSetPath()));

  // Store the image JSON data for assigning via the Contents.json
  const imagesJson: ContentsJson["images"] = await generateUniversalIconAsync(
    projectRoot,
    {
      icon,
  ...
  await writeContentsJsonAsync(join(iosNamedProjectRoot, getImageSetPath()), {
    images: imagesJson,
  });

  console.log("end", iosNamedProjectRoot, getImageSetPath());
}
...
export async function generateUniversalIconAsync(
  ...
  const assetPath = join(iosNamedProjectRoot, getImageSetPath(), filename);
